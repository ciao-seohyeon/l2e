...
class _TransformerTask(base.Task):
  ...
  def _hk_forward(self, batch):
    ...
    return jnp.sum(loss * mask) / jnp.sum(mask) , logits
  ...
  def loss(self, params, key, data):
    l2_norm = 0. # 0.5 * self.weight_decay * sum([jnp.sum(p**2) for p in jax.tree_util.tree_leaves(params)])
    loss, _ = self._net.apply(params,key, data)
    return loss + l2_norm

  def logit(self, params, key, data):
    _ , logits = self._net.apply(params,key, data)
    return logits
  
  def nll(self, params, key, data):
    nll1, _, _ = self._net.apply(params,key, data)
    return nll1
