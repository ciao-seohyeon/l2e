...
from outer_trainers import common
from outer_trainers import gradient_learner
from outer_trainers import truncated_step as truncated_step_mod
from outer_trainers import truncation_schedule as truncation_schedule_mod
import numpy as onp
...
def traj_loss_antithetic_es(
  ...
  if not is_valid:
    ...
    if loss_type == "avg":
      pos_loss = jnp.mean(p_ys.valid_loss, axis=0)
      neg_loss = jnp.mean(n_ys.valid_loss, axis=0)
    elif loss_type == "min":
      pos_loss = jnp.min(p_ys.valid_loss, axis=0)
      neg_loss = jnp.min(n_ys.valid_loss, axis=0)
    elif loss_type == "last":
      pos_loss = p_ys.valid_loss[-1, :]
      neg_loss = n_ys.valid_loss[-1, :]
    else:
      raise ValueError(f"Unsupported loss type {loss_type}.")
  ...
  def compute_gradient_estimate(
    ...
    with profile.Profile("computing_loss_and_update"):
      if self.loss_type in ["avg", "min", "last"]:
        ...
        mean_valid_loss, _, _ = traj_loss_antithetic_es(
            p_yses,
            n_yses,
            vec_pos,
            self.std,
