...
def traj_loss_antithetic_es(
  ...
  def make_logit(param,task ,data):
    key = jax.random.PRNGKey(0) # dummy key
    logit = task.logit(param,key,data)
    return logit
  ...
  if not is_valid:
    ...
    neg_loss = function_bma(n_ys)
  else: # TODO : ADD VALID LOGIT AND VALID DATA 
    pos_loss = function_bma(p_ys)
    neg_loss = function_bma(n_ys)
...
@gin.configurable
class FullES_function_BMA(gradient_learner.GradientEstimator):
...
@gin.configurable
class PMAPFullES(FullES_function_BMA):
  """Gradient Est.
