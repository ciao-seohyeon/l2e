...
from outer_trainers import full_es
from outer_trainers import truncated_step
...
def progress_or_reset_inner_opt_state(
  ...
  def true_fn(key):
    ...
    t = task_family.task_fn(task_param)
    ...
    batch_size = 64 # data['image'].shape[0] 
    return next_inner_opt_state, task_param, jnp.asarray(0), jnp.asarray(0.) , jnp.ones((64,10)) # we do not use this logit#jnp.ones((batch_size, t.datasets.extra_info["num_classes"]))
  ...
  def false_fn(key):
    ...
    logits = task.logit(p,key1,data)
    return next_inner_opt_state, task_param, next_inner_step, jnp.asarray(
        meta_loss, dtype=jnp.float32) , jnp.ones((64,10)) 

  next_inner_opt_state, task_param, next_inner_step, meta_loss , logits = cond_fn(  # pytype: disable=wrong-arg-types  # jax-types
      jnp.logical_not(is_done), false_fn, true_fn, key)

  return next_inner_opt_state, task_param, next_inner_step, meta_loss , jnp.ones((64,10)) # logits
...
def _truncated_unroll_one_step(
  ...
  next_inner_opt_state, task_param, next_inner_step, l , logit = (
...
class VectorizedLOptTruncatedStep(truncated_step.VectorizedTruncatedStep,
  ...
  def unroll_step(self,
    ...
    if meta_data is not None:
      ...
      # valid_loss, aux = vectorized_loss_and_aux(self.task_family, self.learned_opt,
      #                                     theta,
      #                                     next_unroll_state_.inner_opt_state,
      #                                     next_unroll_state_.task_param,
      #                                     vec_keys, meta_data)
      if self.meta_loss_with_aux_key:
        ...
        ys = ys.replace(valid_loss = loss ) # valid_loss)
    @jax.vmap
